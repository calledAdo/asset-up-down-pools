...
/** Reserved, unused: CLOSE destroys the PoolCell (1→0), so no cell carries it.
 *  Value 3 stays reserved to keep the VOID/FINALIZED encodings pinned. */
...
export const CLOSE_GRACE_MIN_SECS = 60n * 60n; // 1 hour
export const CLOSE_GRACE_MAX_SECS = 7n * 24n * 60n * 60n; // 7 days
export const CLOSE_GRACE_MULT = 8n;

// --- PoolData byte lengths ---
export const POOL_LEN_CKB = 173;
export const POOL_LEN_XUDT = 237;
...
/**
 * Teardown grace before a FINALIZED/VOID pool may be CLOSEd:
 * `clamp(duration * 8, 1h, 7d)`. Mirrors `constants::close_grace` in Rust.
 */
export function closeGrace(durationSecs: bigint): bigint {
  const g = durationSecs * CLOSE_GRACE_MULT;
  if (g < CLOSE_GRACE_MIN_SECS) return CLOSE_GRACE_MIN_SECS;
  if (g > CLOSE_GRACE_MAX_SECS) return CLOSE_GRACE_MAX_SECS;
  return g;
}
