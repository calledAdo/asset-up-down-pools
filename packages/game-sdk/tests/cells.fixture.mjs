...
test("asShare matches pool_type_hash ++ side and reads the amount", () => {
  ...
  assert.deepEqual(asShare(up, DEPLOY, POOL_TYPE_HASH), { side: SIDE_UP, amount: 500n });

  const down = { ...up, type: { ...up.type, args: POOL_TYPE_HASH + "02" } };
  assert.deepEqual(asShare(down, DEPLOY, POOL_TYPE_HASH), { side: SIDE_DOWN, amount: 500n });

  // Wrong pool, and a non-share cell.
  assert.equal(asShare(up, DEPLOY, "0x" + "99".repeat(32)), null);
  assert.equal(asShare({ ...up, type: null }, DEPLOY, POOL_TYPE_HASH), null);
});

test("asTreasury matches the lock binding and reads the amount", () => {
  const t = {
    type: { codeHash: "0x" + "aa".repeat(32), hashType: "type", args: "0x" },
    lock: { codeHash: DEPLOY.treasuryLockCodeHash, hashType: "data2", args: POOL_TYPE_HASH },
    data: encodeAmount(777n),
  };
  assert.equal(asTreasury(t, DEPLOY, POOL_TYPE_HASH), 777n);
  // Wrong pool binding, or no type script.
  assert.equal(asTreasury(t, DEPLOY, "0x" + "99".repeat(32)), null);
  assert.equal(asTreasury({ ...t, type: null }, DEPLOY, POOL_TYPE_HASH), null);
});
