...
export function asShare(
  ...
  if (!cell.type || !eqHashLower(cell.type.codeHash, deploy.shareXudtCodeHash)) return null;
 ...
 * lock binding (`lock == { treasury_lock, args: poolTypeHash }`) uniquely
 * identifies the pool's treasury; the staked-asset identity is fixed by the
 * pool's config.
 */
export function asTreasury(cell: CellView, deploy: PoolDeployment, poolTypeHash: Hex): bigint | null {
  if (!eqHashLower(cell.lock.codeHash, deploy.treasuryLockCodeHash)) return null;
