...
## 2. Use a dedicated oracle cell (one per feed)
...
Updated only at boundaries (≈2× per pool), so it's cheap.

**One cell per *feed*, shared across all its cadences** (revised — earlier drafts said one
cell per lane). Back-to-back pools in a lane have monotonically increasing boundaries
(`start₁ < close₁ = start₂ < …`), and different cadences of a feed only ever need a tick at
*coincident* grid boundaries — where they need the **same** tick (the first tick ≥ that
boundary, since every coarse boundary is also a fine one). So one forward-advancing cell
serves every cadence of a feed without contention. This is also what lets coincident
transitions from different cadences **batch into one tx**: `find_oracle` locates the cell by
`feed_id` and rejects two same-feed oracle deps in a single tx, so per-lane cells would
*prevent* the batching. Example: at the top of the hour a **5m RESOLVE and a 1h ACTIVATE**
both read the one cell version holding the first tick ≥ that boundary, in one tx.
Cross-*feed* transitions (BTC + ETH) never share a tx — different cells.
...
The **lower bound is inclusive, the upper strict**: a tick at `publish_time == start_time`
(or `== close_time`) is the canonical boundary price and already proves real time reached the
boundary (Pyth can't sign the future), so it's accepted; the strict upper keeps each phase from
